...
const LogoHistory = () => {
  ...
  const fetchLogoHistory = useCallback(async () => {
    ...
    try {
      ...
      console.log("Fetching logo history for user:", userId);
  ...
  const fetchLogoData = useCallback(async (logoId) => {
    ...
    if (activeImageFetches[logoId]) {
      console.log(`Already fetching logo ${logoId}, skipping duplicate request`);
      return null;
    }
    
    // Check if we already have this logo in cache
    if (logoImageCache[logoId]) {
      console.log(`Using cached data for logo ${logoId}`);
    ...
    try {
      ...
      console.log("Fetching image data for logo:", logoId);
      ...
      if (response && response.success) {
        console.log(`Successfully fetched image data for logo ${logoId}`);
  ...
  const handleViewDetails = useCallback((logo) => {
    if (!logo || !logo.id) {
      console.error("Invalid logo selected");
      return;
    }
    
    console.log("Viewing details for logo:", logo.id);
  ...
  const handleDownload = useCallback((logo) => {
    if (!logo || !logo.id) {
      console.error("Invalid logo for download");
      return;
    }
    ...
    if (imageData) {
      try {
        console.log("Downloading logo with base64 data");
      ...
      console.log("Fetching data before download for logo:", logo.id);
  ...
  useEffect(() => {
    ...
    const loadLogoImagesInBatches = async () => {
      ...
      for (let i = 0; i < logosToLoad.length; i += BATCH_SIZE) {
        ...
        // Process this batch
        console.log(`Loading batch ${i/BATCH_SIZE + 1} of ${Math.ceil(logosToLoad.length/BATCH_SIZE)}`);
              ...
              {selectedLogoData ? (
                <div className="flex items-center justify-center p-4 max-h-[50vh] overflow-hidden">
                  <motion.img 
                    ...
                    onError={(e) => {
                      console.error("Detail image failed to load");
